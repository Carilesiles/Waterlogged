...
function FreelanceMerc_PromptMenu(currentClass, activator, caller)
    local menu =  {
        ...
        onSelect = function(player, index)
            ...
            ---@diagnostic disable-next-line: undefined-global
            timer.Simple(SWITCH_COOLDOWN, function() cooldowns[handleIndex] = nil end)
        end,
        onCancel = nil,
    }

    for index, className in pairs(classIndices) do
        menu[index] = {text = className, value = index,
        disabled = classIndices_Internal[currentClass] == className}
    end
