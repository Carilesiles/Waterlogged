...
local RAGE_OVERRIDES_APPLY = {
	["throwable fire speed"] = function(activator)
		...
		primary:SetAttributeValue("explosive bullets", 147)
		primary:SetAttributeValue("explosion particle", "ExplosionCore_sapperdestroyed")
		primary:SetAttributeValue("mult dmg vs tanks", 1.5)
		primary:SetAttributeValue("rocket jump damage reduction", 0.3)
	end,
}
local RAGE_OVERRIDES_REMOVE = {
	["throwable fire speed"] = function(activator)
		local primary = activator:GetPlayerItemBySlot(LOADOUT_POSITION_PRIMARY)
		primary:SetAttributeValue("explosive bullets", nil)
		primary:SetAttributeValue("explosion particle", nil)
		primary:SetAttributeValue("mult dmg vs tanks", nil)
		primary:SetAttributeValue("rocket jump damage reduction", nil)
	end,
}
...
function KnockbackRageStart(_, activator)
	...
	knockbackHandles[handleIndex] = timer.Create(UPDATE_INTERVAL, function()
		...
		if draining then
			...
			if fakeMeter <= 0 then
				activator
					:GetPlayerItemBySlot(LOADOUT_POSITION_PRIMARY)
					:SetAttributeValue("increase buff duration HIDDEN", nil)
